...
import {
    ...
    PADDLE_EDGE_INSET,
    MORPH_MS,
    ACTIVATE_TAPS_REQUIRED,
    ACTIVATE_WINDOW_MS,
    PADDLE_HOLD_SPEED,
    MAX_BALL_SPEED,
    LANDSCAPE_BALL_SPEED,
    MIN_BALL_VX,
    ARROW_SIZE,
    ARROW_GAP,
    ARROW_COURT_GAP,
    ...
    COMMENT_GAP,
    EYE_TAP_ZONE_MIN_W,
    EYE_TAP_ZONE_W_RATIO,
    EYE_TAP_ZONE_MIN_H,
    EYE_TAP_ZONE_H_RATIO,
    EYE_TAP_ZONE_OUTSET,
    MAX_PONG_GAZE,
    DESKTOP_COURT_RADIUS,
    DESKTOP_COURT_RADIUS_PX,
    IOS_COURT_RADIUS,
    IOS_COURT_RADIUS_PX,
    DESKTOP_HUD_GAP,
    SERVE_COUNTDOWN_MS,
    RESERVE_AFTER_MISS_MS,
    PONG_ARM_HINT_IDLE_MS,
    PONG_QUIT_COMMENT,
    SIX_SEVEN_DANCE_MS,
    KEY_HOLD,
    KEYBOARD_ARM_SEQUENCE,
    RALLY_HIT_MIN,
    EDGE_REL_THRESHOLD,
    EDGE_COMMENT_CHANCE,
    WHIFF_COMMENT_CHANCE,
    MILESTONE_SCORES,
    STRAIGHT_REL_THRESHOLD,
    STRAIGHT_STREAK_MIN,
} from './constants.js';
...
function courtPlaySpan() {
    return courtPlayBottom() - courtPlayTop();
}
...
function setPaddleVertical(el, x, cy, half) {
    if (!el) return;
...
function syncPaddles(half, now = performance.now()) {
    ...
    setPaddleVertical(paddleRightEl, paddleRightX(), rightY, half);
}
...
function activatePong() {
    ...
    startPongLoop();
    startServeCountdown();
    callHook('recordBehavior', 'pong_session');
...
export function pongBlocksArrowNav(e) {
    ...
    if (getHook('konamiBlocksPongArming')?.()) return false;
...
function bindKeyboardControls() {
    window.addEventListener('keydown', (e) => {
        ...
        if (!e.repeat && canArmPongFromKeyboard()) {
            if (getHook('konamiClaimsKey')?.(e)) return;
